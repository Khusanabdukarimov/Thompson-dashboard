...
export default function KunlikPage() {
  ...
  const source: SourceKey =
    activePreset === "target" || activePreset === "instagram"
      ? activePreset
      : "all";
  ...
  function valueFor(
    ...
    if (metric.key === "qual_pct") {
      ...
      if (typeof qual !== "number" || typeof leads !== "number" || leads === 0)
        return undefined;
          ...
          <div className="bg-bg2 border border-border rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b border-border">
              <div className="text-[14px] font-semibold">
                Kunlik ma'lumotlar jadvali
              </div>
              <div className="text-[11px] text-text3 mt-0.5">
                Yashil — avtomatik · Sariq — qo'lda · Bo'sh — kiritilmagan ·
                Bugun ustun ko'k bilan ajratilgan
              </div>
            </div>
              ...
              <table className="w-full border-collapse text-[12px]">
                <thead>
                  <tr className="bg-bg3 text-text3 uppercase tracking-wider text-[10.5px] font-semibold">
                    ...
                    <th className="text-right px-3 py-2 min-w-[88px] border-b border-border">
                      {period === "all" ? "Oylik" : "Davr jami"}
                    </th>
                    {Array.from({ length: days }, (_, i) => i + 1).map((d) => {
                      const isToday =
                        isCurrentMonth(month, year) && d === todayDay;
                      const dim = !mask[d - 1];
                      return (
                        <th
                          key={d}
