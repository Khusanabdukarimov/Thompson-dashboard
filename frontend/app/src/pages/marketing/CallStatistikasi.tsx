...
const AVATAR_COLORS = [
  ...
  "#F44336","#009688","#3F51B5","#E91E63","#00BCD4","#FF5722",
];
const avatarColor = (id: number) => AVATAR_COLORS[id % AVATAR_COLORS.length];

// ── Avatar (photo or initials) ────────────────────────────────────
function Avatar({ name, photoUrl, id }: { name: string; photoUrl: string | null; id: number }) {
...
// ── Metric Card ───────────────────────────────────────────────────
function Card({ label, value, sub, icon, iconBg, badge, badgeColor, valueColor }: {
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 8 }}>
        ...
        {badge && (
          <span style={{
            ...
            background: `${badgeColor}18`, border: `1.5px solid ${badgeColor}35`,
            borderRadius: 6, padding: "2px 8px", lineHeight: 1.7, marginBottom: 2,
          }}>{badge}</span>
        )}
      </div>
      {sub && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <label style={{ fontSize: 12, color: "var(--text2)", display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <input type="date" value={startDate} onChange={(e) => onStartDate(e.target.value)}
            style={{ border: "1px solid var(--border)", borderRadius: 8, padding: "7px 10px", fontSize: 13, background: "var(--bg2)", color: "var(--text)" }} />
        </label>
        <label style={{ fontSize: 12, color: "var(--text2)", display: "flex", flexDirection: "column", gap: 4 }}>
          Tugash sanasi
...
function Delta({ curr, prev }: { curr: number; prev: number | undefined }) {
  if (prev === undefined) return <span style={{ color: "var(--text2)" }}>—</span>;
            ...
            return (
              <tr key={c.id} style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: "8px 14px" }}>
                  {ct ? <span style={{ fontSize: 11, fontWeight: 600, color: ct.color, background: `${ct.color}15`, border: `1px solid ${ct.color}30`, borderRadius: 5, padding: "2px 8px" }}>{ct.label}</span> : "—"}
                </td>
                ...
                <td style={{ padding: "8px 14px", color: "var(--text2)", whiteSpace: "nowrap" }}>
                  {c.call_start ? new Date(c.call_start).toLocaleString("ru-RU", { day: "2-digit", month: "2-digit", year: "2-digit", hour: "2-digit", minute: "2-digit" }) : "—"}
                </td>
                <td style={{ padding: "8px 14px" }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: ok ? "#4CAF50" : "#F44336", background: ok ? "#4CAF5015" : "#F4433615", border: `1px solid ${ok ? "#4CAF5030" : "#F4433630"}`, borderRadius: 5, padding: "2px 8px" }}>
                    {ok ? "Muvaffaqiyatli" : "Muvaffaqiyatsiz"}
                  </span>
                </td>
                <td style={{ padding: "8px 14px" }}>
                  {c.lead_id
                    ? <a href={`https://mountain.bitrix24.kz/crm/lead/details/${c.lead_id}/`} target="_blank" rel="noreferrer" style={{ color: "#2196F3", textDecoration: "none", fontSize: 12 }}>{c.lead_title || `#${c.lead_id}`}</a>
                    : "—"}
                </td>
              </tr>
            );
...
export default function CallStatistikasi() {
  ...
  // Global stats (ne_perezvonili, reaksiya_vaqti)
  const globalQ = useQuery({
    queryKey: ["call-global-stats", startDate, endDate],
    queryFn:  () => getCallGlobalStats({ start_date: startDate, end_date: endDate }),
  });

  // Previous period (for dynamics)
  const [prevFrom, prevTo] = useMemo(() => prevPeriod(startDate, endDate), [startDate, endDate]);
  const prevStatsQ = useQuery({
    queryKey: ["call-stats-prev", prevFrom, prevTo],
    queryFn:  () => getCallStats({ start_date: prevFrom, end_date: prevTo }),
  });

  const rows: CallStatsRow[] = statsQ.data ?? [];

  // Build prev lookup by responsible_id
  const prevMap = useMemo(() => {
    const m = new Map<number, CallStatsRow>();
    (prevStatsQ.data ?? []).forEach((r) => m.set(r.responsible_id, r));
    return m;
  }, [prevStatsQ.data]);
  ...
  const btnStyle: React.CSSProperties = {
    ...
    background: "var(--bg)", color: "var(--text2)", fontSize: 13, cursor: "pointer", fontWeight: 500,
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg2)" }}>
      <Topbar
        ...
        actions={
          <div style={{ display: "flex", gap: 8, position: "relative" }}>
            <button onClick={doSyncPhotos} disabled={syncingPhotos} style={btnStyle} title="Foydalanuvchi rasmlarini sinxronlashtirish">
              <PhoneCall size={14} />
              {syncingPhotos ? "Rasmlar..." : "Rasmlar sync"}
            </button>
            <button onClick={doSync} disabled={syncing} style={btnStyle}>
              <RefreshCw size={14} style={{ opacity: syncing ? 0.5 : 1 }} />
              {syncing ? "Sinxronizatsiya..." : "Sinxronizatsiya"}
            </button>
            <div style={{ position: "relative" }}>
              <button onClick={() => setFilterOpen((v) => !v)} style={{ ...btnStyle, color: filterOpen ? "#2196F3" : "var(--text2)", borderColor: filterOpen ? "#2196F3" : "var(--border)" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          ...
          <Card label="Chiquvchi qo'ng'iroq"
          ...
          <Card label="Kiruvchi qo'ng'iroq"
            value={<>{totals.inbound} <span style={{ fontSize: 16, fontWeight: 500, color: "var(--text2)" }}>ta</span></>}
            sub={fmtDur(totals.inbound_dur)}
            icon={<PhoneIncoming size={19} color="#E91E63" />} iconBg="rgba(233,30,99,0.10)" />
          <Card label="Muvaffaqiyatli"
        ...
        <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
            ...
            <button title="Export" style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 34, height: 34, borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg2)", color: "var(--text2)", cursor: "pointer" }}>
              <Download size={15} />
            </button>
          </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ ...TH, color: "#2196F3", borderLeft: "2px solid rgba(33,150,243,0.15)" }} colSpan={3}>QO'NG'IROQLAR SONI</th>
                    <th style={{ ...TH, color: "#4CAF50", borderLeft: "2px solid rgba(76,175,80,0.15)" }} colSpan={3}>UNIKAL QO'NG'IROQLAR</th>
                    <th style={{ ...TH, color: "#9C27B0", borderLeft: "2px solid rgba(156,39,176,0.15)" }} colSpan={3}>DAVOMIYLIK</th>
                    <th style={{ ...TH, color: "#FF9800", borderLeft: "2px solid rgba(255,152,0,0.15)" }} colSpan={2}>NATIJA</th>
                    <th style={{ ...TH, color: "var(--text2)", borderLeft: "2px solid rgba(0,0,0,0.06)" }} rowSpan={2}>DINAMIKA</th>
                  </tr>
                  <tr>
                    ...
                    <th style={TH}>Jami</th>
                    <th style={{ ...TH, borderLeft: "2px solid rgba(255,152,0,0.15)" }}>Обратные</th>
                ...
                <tbody>
                  {rows.map((u) => {
                    ...
                    const prev  = prevMap.get(u.responsible_id);
                    return (
                      <>
                        <tr
                          key={u.responsible_id}
                          style={{ background: isSel ? "rgba(33,150,243,0.06)" : "var(--bg)", cursor: "pointer" }}
                        ...
                        >
                          ...
                          <td style={{ ...TD, color: u.ne_perezvonili > 0 ? "#F44336" : "var(--text)" }}>{u.ne_perezvonili}</td>

                          {/* Dinamika */}
                          <td style={{ ...TD, borderLeft: "2px solid rgba(0,0,0,0.05)" }}>
                            <Delta curr={u.total_calls} prev={prev?.total_calls} />
                          </td>
                        </tr>
