...
export default function KpiRulesPage() {
  ...
  return (
    <>
      <Topbar
        title="KPI qoidalar"
        sub={`${q.data?.count ?? 0} ta qoida · darajali komissiya`}
        actions={
        ...
        {q.isLoading && !q.data ? (
          <div className="grid grid-cols-2 gap-3">
            {Array.from({ length: 2 }).map((_, i) => (
              <div
                key={i}
                className="bg-bg2 border border-border rounded-lg shadow p-4"
              >
...
function RuleCard({ rule, onEdit }: { rule: KpiRule; onEdit: () => void }) {
  ...
  async function handleDelete() {
    if (!confirm(`"${rule.name}" qoidasini o'chirishni tasdiqlaysizmi?`))
      return;
    try {
  ...
  return (
    <div className="bg-bg2 border border-border rounded-lg shadow overflow-hidden">
      ...
      <div className="p-3 space-y-1">
        {rule.tiers.length === 0 && (
          <div className="text-text3 text-[12px] text-center py-4">
            Tier yo'q
          </div>
        )}
        {rule.tiers.map((t, i) => {
          ...
          return (
            <div
              key={i}
              className="flex items-center gap-3 py-1.5 px-2.5 rounded-md bg-bg3 border border-border"
            >
              <span className="mono text-[12px] font-medium min-w-[120px]">
                {range}
              </span>
              <div className="flex-1 h-1.5 bg-bg4 rounded overflow-hidden">
                <div
                  className="h-full rounded bg-gradient-to-r from-blue-2 to-cyan-400"
                  style={{ width: `${(Number(t.percent) / max) * 100}%` }}
                />
              </div>
              <span className="mono text-[13px] font-bold text-green min-w-[34px] text-right">
                {Number(t.percent)}%
              </span>
            </div>
          );
        ...
        <div className="text-[10px] text-text3 mt-2 text-center">
          Yagona daraja: butun summa × mos tier % (misol: $41,200 → 15% →
          $6,180)
        </div>
      </div>
    </div>
  ...
  function addTier() {
    setForm((f) => {
      ...
      return {
        ...
        tiers: [
          ...f.tiers,
          { from: newFrom, to: null, percent: (last?.percent ?? 0) + 1 },
        ],
      };
    });
  }
  ...
  async function save() {
    ...
    try {
      const cleanTiers = form.tiers.map((t) => ({
        ...
        to:
          t.to == null || t.to === undefined || (t.to as unknown) === ""
            ? null
            : Number(t.to),
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-bg2 border border-border rounded-xl p-6 w-[560px] max-h-[88vh] overflow-y-auto shadow-lg z-[301]">
          <Dialog.Title className="text-[15px] font-semibold mb-4">
            {rule ? "Qoida tahrirlash" : "Yangi KPI qoida"}
          </Dialog.Title>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Nomi" className="col-span-2">
              <input
                ...
                placeholder="Sales KPI — Closer roli"
              />
            </Field>
            <Field label="Rol">
              <select
            ...
            <Field label="Hisob rejimi" className="col-span-2">
              ...
              >
                <option value="single_tier">
                  Yagona daraja — butun summa × mos tier %
                </option>
                <option value="multi_tier">
                  Ko'p daraja — progressiv (har tier o'z foizi bilan)
                </option>
              </select>
          ...
          <div className="mt-4 mb-2 flex items-center justify-between">
            <span className="text-[12px] font-semibold uppercase tracking-wider text-text2">
              Tierlar
            </span>
            <Button size="sm" onClick={addTier}>
              <Plus className="w-3 h-3" /> Tier qo'shish
            </Button>
          </div>
            ...
            {form.tiers.map((t, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <input
                  type="number"
                  className={`${fi} flex-1`}
                  placeholder="Gacha (∞ uchun bo'sh qoldiring)"
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTier(i, { percent: Number(e.target.value) })
                  }
                />
                <span className="text-text3 text-[11px]">%</span>
                <Button
...
const fi =
  "px-2.5 py-2 rounded-[7px] border border-border bg-bg text-text text-[12.5px] focus:outline-none focus:border-blue focus:bg-bg2 focus:shadow-[0_0_0_3px_rgba(34,102,245,0.1)]";
