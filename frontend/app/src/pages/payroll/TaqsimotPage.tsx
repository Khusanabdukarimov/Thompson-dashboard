...
export default function TaqsimotPage() {
  const empQ = useQuery({
    queryKey: ["taqsimot/employees"],
    queryFn: listEmployees,
  });
  const kpiQ = useQuery({
    queryKey: ["taqsimot/kpi-rules"],
  ...
  const columns = useMemo<ColumnDef<Employee, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        id: "taqsimot_pct",
        enableSorting: false,
        cell: () => <span className="text-text3">—</span>,
      },
      {
        header: "Status",
        accessorKey: "status",
  ...
  return (
    <>
      <Topbar
        title="Taqsimot"
        sub={`${empQ.data?.count ?? 0} ta xodim · ish haqi taqsimoti`}
        actions={
          <>
            <input
...
function EditModal({
  ...
  async function save() {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ["taqsimot/employees"] });
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Fix base (oy, so'm)">
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, fix_base_uzs: Number(e.target.value) }))}
              />
            </Field>
            <Field label="Davomat haftalik (so'm)">
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, attendance_weekly_uzs: Number(e.target.value) }))}
              />
            </Field>
            <Field label="Hisobot haftalik (so'm)">
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, report_weekly_uzs: Number(e.target.value) }))}
              />
            </Field>
            <Field label="KPI qoida">
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, kpi_rule_id: e.target.value ? Number(e.target.value) : null }))
                }
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Dashboard roli" className="col-span-2">
                <select
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, dashboard_role: e.target.value }))}
                >
