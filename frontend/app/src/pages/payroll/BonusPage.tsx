...
export default function BonusPage() {
  ...
  const ROLE_TABS = ['closer', 'hunter', 'dizayner', 'neyming', 'sayohat'];
  const [activeTab, setActiveTab] = useState('closer');
  const tabRules = allRules.filter(r => r.target_role === activeTab);

  return (
    <>
      <Topbar
        title="Bonus Qoidalari"
        sub="Global sozlamalar"
        actions={
          <>
            <select className="px-2.5 py-1.5 rounded border border-border2 bg-bg2 text-[12px] shadow-xs" value={period} onChange={e => setPeriod(e.target.value)}>
      ...
      <div className="flex-1 overflow-y-auto px-[22px] py-[18px] bg-bg">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            { label: 'FAOL BONUS QOIDALARI', value: activeRulesCount, sub: `+${allRules.length - activeRulesCount} yangi` },
            { label: 'OYLIK BONUS BUDJETI', value: `$${Math.round(totalBudget).toLocaleString()}`, sub: 'Tasdiqlangan' },
            { label: 'QAMRAB OLINGAN', value: coveredIds.size || employees.length, sub: 'Shtadagi xodimlar' },
            { label: "O'RTACHA BONUS", value: avgBonus > 0 ? `$${Math.round(avgBonus)}` : '0', sub: 'Bir kishiga' },
          ].map(m => (
            <div key={m.label} className="bg-bg2 border border-border rounded-xl p-4">
              <div className="text-[10.5px] font-semibold text-text3 tracking-wider mb-3">{m.label}</div>
              <div className="text-[24px] font-bold text-text mb-1">{m.value}</div>
              <div className="text-[11.5px] text-text3">{m.sub}</div>
            </div>
          ))}
        </div>

        {/* Role tabs */}
        <div className="flex border-b border-border mb-0 gap-0">
          {ROLE_TABS.map(t => (
            <button
              key={t}
              onClick={() => setActiveTab(t)}
              className={`px-5 py-2.5 text-[13px] font-semibold border-b-2 transition-all -mb-px ${
                activeTab === t ? 'border-blue text-blue' : 'border-transparent text-text2 hover:text-text'
              }`}
            >{t.charAt(0).toUpperCase() + t.slice(1)}{t === 'sayohat' ? ' bonuslari' : ''}</button>
          ))}
        </div>

        {/* Table */}
        <div className="bg-bg2 border border-border border-t-0 rounded-b-xl overflow-hidden mb-5">
          <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
            <span className="text-[14px] font-bold text-text">{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} bonuslari</span>
          </div>
          <DataTable<BonusRule>
            columns={ruleColumns}
            data={tabRules}
        ...
        <div className="bg-bg2 border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
            <span className="text-[14px] font-bold text-text">Berilgan bonuslar — {period}</span>
            <span className="text-[12px] text-text3">{awardsQ.data?.count ?? 0} ta</span>
          </div>
