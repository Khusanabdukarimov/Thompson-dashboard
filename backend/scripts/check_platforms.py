...
res = requests.get(
    ...
    params={
        ...
        "fields": "publisher_platform,spend,actions"
    }
)
data = res.json().get('data', [])
# Filter for instagram and facebook
for row in data:
    if row.get('publisher_platform') in ['facebook', 'instagram']:
        print(f"Platform: {row.get('publisher_platform')}, Spend: {row.get('spend')}")
