...
async function main() {
  ...
  try {
    ...
    for (const [uzName, bitrixId] of Object.entries(STAGE_MAP)) {
      ...
      await client.query(
        `INSERT INTO stages (bitrix_id, name, entity, sort_order)
         VALUES ($1, $2, 'lead', 0)
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
      ...
      for (const r of batch) {
        ...
        try {
          ...
          // Clear old phones just to be safe
          await client.query(`DELETE FROM lead_phones WHERE lead_id = $1`, [id]);
    ...
    await client.query('COMMIT');
    console.log(`\n\n✓ Done. ${inserted} leads imported, ${skipped} skipped.`);
