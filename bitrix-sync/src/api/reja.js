  ...
  try {
    ...
    const actualsRes = allIds.length ? await pool.query(`
      ...
      WHERE d.responsible_id = ANY($1)
        AND COALESCE(d.uf_sale_date, d.closedate, d.date_modify, d.date_create)::date BETWEEN $2 AND $3
...
router.get('/plans/:id/progress', async (req, res) => {
  ...
  try {
    ...
    const actualsRes = await pool.query(`
      SELECT
        d.responsible_id,
        COALESCE(d.uf_sale_date, d.closedate, d.date_modify, d.date_create)::date::text AS close_date,
        SUM(d.opportunity)::numeric                                      AS amount
      FROM deals d
      JOIN stages s ON s.id = d.stage_id AND s.is_won = TRUE
      WHERE d.responsible_id = ANY($1)
        AND COALESCE(d.uf_sale_date, d.closedate, d.date_modify, d.date_create)::date BETWEEN $2 AND $3
      GROUP BY d.responsible_id, COALESCE(d.uf_sale_date, d.closedate, d.date_modify, d.date_create)::date
