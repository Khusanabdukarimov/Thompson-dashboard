    ...
    const { rows } = await pool.query(
      `WITH fl AS (
       ...
       SELECT
         ...
         COUNT(fl.id) FILTER (WHERE fl.stage_bid NOT IN ('JUNK','UC_L8G2B9'))::int                               AS sifatli_lid_count,
           ...
           COUNT(l.id) FILTER (WHERE s.bitrix_id IN ('JUNK'))::int                AS muvaffaqiyatsiz_count
         FROM leads l
         JOIN stages s ON s.id = l.stage_id
         WHERE ${statsWhere}`,
        statsParams
    ...
    const { rows } = await pool.query(
      `WITH fl AS (
       ...
       SELECT
         ...
         COUNT(fl.id) FILTER (WHERE fl.stage_bid = 'IN_PROCESS')::int                         AS target_uc,
         COUNT(fl.id) FILTER (WHERE fl.stage_bid = 'UC_6INRIS')::int                          AS target_kids,
         COUNT(fl.id) FILTER (WHERE fl.stage_bid = 'UC_YGM8H2')::int                          AS target_mkt,
         COUNT(fl.id) FILTER (WHERE fl.stage_bid = 'UC_TO2TYK')::int                          AS target_bog,
    ...
    const { rows } = await pool.query(
      `WITH fl AS (
       ...
       SELECT
         ...
         COUNT(fl.id) FILTER (WHERE fl.stage_bid NOT IN ('JUNK','UC_L8G2B9'))::int                               AS sifatli_lid,
         COUNT(fl.id) FILTER (WHERE fl.stage_bid IN ('JUNK'))::int                AS sifatsiz_lid,
         COUNT(fl.id) FILTER (WHERE fl.stage_bid IN ('UC_L8G2B9'))::int             AS bekor_boldi,
  ...
  try {
    const { rows } = await pool.query(
      `SELECT
         ...
         COUNT(*) FILTER (WHERE s.bitrix_id NOT IN ('JUNK','UC_L8G2B9'))::int AS sifatli_lid,
         ...
         COUNT(*) FILTER (WHERE s.bitrix_id IN ('JUNK'))::int AS sifatsiz,
         COUNT(*) FILTER (WHERE s.bitrix_id IN ('UC_L8G2B9'))::int AS bekor_boldi,
  ...
  try {
    const { rows } = await pool.query(
      `SELECT
         ...
         (s.bitrix_id IN ('UC_L8G2B9'))::int                            AS bekor_boldi,
